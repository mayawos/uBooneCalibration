#define X_calibration_DataDrivenMap_plane2_cxx
#include "X_calibration_DataDrivenMap_plane2.h"
#include <TH1.h>
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void X_calibration_DataDrivenMap_plane2::Loop(int corr, int i_year, int i_month, int i_date, int f_date, std::string sample)
{
  std::cout << "///////////////////////////////////// Generating X correction histogrmas for different dates and all planes ///////////////////////////////////" << std::endl;
  
  ifstream infile2;
  infile2.open("goodruns_intersec_all_runs_mcc9_run4.txt");
  //infile2.open("goodruns_intersec_all_runs_mcc9.txt");
  //infile2.open("common_events.list");
  //infile2.open("low_purity_r4.txt");
  ...
  for(int plane_number=2; plane_number<3; plane_number++){
    ofstream outfile;
    outfile.open(Form("run4_external_on_beam_data_cross_numbers_month%d_plane%dSCE.txt",i_month,plane_number));
    std::cout  << "********** MONTH : " << i_month << " ******************" << std::endl;
    outfile<<i_month<<"/";
    ifstream infile;
    infile.open(Form("dates_of_month_%d.txt",i_month));
    for( int my_date=i_date; my_date < f_date+1; my_date++ ){
      ofstream outfile0;
      outfile0.open(Form("run_%d_%d.txt",i_month, my_date));
